...
管理用户每日/每月请求配额和 API Key RPM 限制。
分布式模式使用 Redis 原子计数器，单节点模式使用数据库表。
"""
...
class QuotaManager:
    ...
    # Redis key prefixes
    _USER_DAILY_KEY = "kirogate:user:{user_id}:daily_count"
    _USER_MONTHLY_KEY = "kirogate:user:{user_id}:monthly_count"
    _APIKEY_RPM_KEY = "kirogate:apikey:{api_key_id}:rpm"
