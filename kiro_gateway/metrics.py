        ...
        if settings.is_distributed:
            ...
            if client:
                try:
                    ...
                    logger.info("Metrics: Redis 状态加载完成")
                except Exception as e:
                    logger.warning(f"Metrics: Redis 状态加载失败: {e}")
    ...
    async def flush(self) -> None:
        ...
        async with self._lock:
            try:
                ...
                logger.debug("Metrics: 本地待同步计数已刷新到 Redis")
            except Exception as e:
                logger.warning(f"Metrics: flush 到 Redis 失败: {e}")
    ...
    async def _export_prometheus_distributed(self) -> str:
        ...
        try:
            ...
            for ep, count in by_endpoint.items():
                lines.append(
                    f'kirogate_requests_total{{endpoint="{ep}"}} {count}'
                )
            ...
            for error_type, count in by_error_type.items():
                lines.append(f'kirogate_errors_total{{type="{error_type}"}} {int(count)}')

            # Retries
            lines.append("# HELP kirogate_retries_total Total number of retries")
            lines.append("# TYPE kirogate_retries_total counter")
            lines.append(f"kirogate_retries_total {total_retries}")

            # Token usage
            lines.append("# HELP kirogate_tokens_total Total tokens used")
            lines.append("# TYPE kirogate_tokens_total counter")
            for model, tokens in input_tokens.items():
                lines.append(f'kirogate_tokens_total{{model="{model}",type="input"}} {int(tokens)}')
            for model, tokens in output_tokens.items():
                lines.append(f'kirogate_tokens_total{{model="{model}",type="output"}} {int(tokens)}')
