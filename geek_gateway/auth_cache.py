...
Manages multiple GeekAuthManager instances for different refresh tokens.
...
# Redis key prefix
_REDIS_KEY_PREFIX = "GeekGate:auth_cache:"


class AuthManagerCache:
    """
    LRU Cache for GeekAuthManager instances.
    ...
    def __init__(self, max_size: int = 100, hot_cache_size: int = 50):
        ...
        self.cache: OrderedDict[str, GeekAuthManager] = OrderedDict()

        # Distributed mode: local hot cache
        self._hot_cache: OrderedDict[str, GeekAuthManager] = OrderedDict()
    ...
    async def get_or_create(
        ...
        Returns:
            GeekAuthManager instance for the refresh token
        """
    ...
    def _put_hot_cache(self, refresh_token: str, auth_manager: GeekAuthManager) -> None:
    ...
    @staticmethod
    def _serialize_manager(auth_manager: GeekAuthManager) -> str:
    ...
    @staticmethod
    def _deserialize_manager(data_str: str) -> GeekAuthManager:
        """Deserialize JSON from Redis into a GeekAuthManager instance."""
    ...
    async def _get_from_redis(
        ...
        try:
            ...
            if client is None:
                logger.warning("Redis 不可用，降级为仅本地热缓�?)
                return None
            ...
            logger.warning(f"。Redis 读取认证缓存失败: {e}，降级为本地缓存")
            return None

    async def _put_to_redis(self, refresh_token: str, auth_manager: GeekAuthManager) -> None:
        ...
        async with self.lock:
            ...
            if settings.is_distributed:
                try:
                    ...
                    logger.warning(f"。Redis 删除认证缓存失败: {e}")
