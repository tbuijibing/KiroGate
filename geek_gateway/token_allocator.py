...
"""
GeekGate 智能 Token 分配�?

实现基于成功率、新鲜度、负载均衡和风控安全?Token 智能分配算法?
支持双模式运行：
- 单节点模式：内存字典 + asyncio.Lock（原有行为）
- 分布式模式：Redis Sorted Set + 分布式锁 + 原子计数?
"""
...
from geek_gateway.database import user_db, DonatedToken
from geek_gateway.auth import GeekAuthManager
from geek_gateway.config import settings


# Redis key constants
_SCORES_KEY = "GeekGate:tokens:scores"
_ALLOC_LOCK_KEY = "GeekGate:token_alloc:lock"
_TOKEN_PREFIX = "GeekGate:token"

# Cooldown policy: consecutive_fails -> cooldown_seconds (None = suspend)
COOLDOWN_POLICY = {
    2: 300,     # 连续失败 2 ??冷却 5 分钟
    3: 1800,    # 连续失败 3 ??冷却 30 分钟
    5: None,    # 连续失败 5 ??暂停
}
...
class SmartTokenAllocator:
    ...
    - 分布式：Redis Sorted Set + 分布式锁 + 原子计数?
    """

    def __init__(self):
        self._lock = asyncio.Lock()
        self._token_managers: dict[int, GeekAuthManager] = {}
    ...
    async def get_best_token(self, user_id: Optional[int] = None) -> Tuple[DonatedToken, GeekAuthManager]:
        """
        获取最?Token?
        ...
        Returns:
            (DonatedToken, GeekAuthManager) tuple

        Raises:
            NoTokenAvailable: 无可?Token（含 retry_after 属性用?429 响应?
        """
    ...
    async def _allocate_local(self) -> Tuple[DonatedToken, GeekAuthManager]:
        """单节点模?Token 分配�?""
    ...
    async def _allocate_distributed(self, user_id: Optional[int] = None) -> Tuple[DonatedToken, GeekAuthManager]:
        """分布式模?Token 分配�?""
    ...
    async def _allocate_with_lock(self, client) -> Tuple[DonatedToken, GeekAuthManager]:
    ...
    async def _allocate_from_cache(self) -> Tuple[DonatedToken, GeekAuthManager]:
    ...
    async def _record_usage_distributed(self, token_id: int, success: bool) -> None:
        ...
        try:
            ...
            logger.debug(f"TokenAllocator: 记录 Token {token_id} 使用结果。Redis 失败: {e}")
    ...
    async def _get_manager(self, token: DonatedToken) -> GeekAuthManager:
        """获取或创?Token 对应?AuthManager（线程安全）�?""
        async with self._lock:
    ...
    async def _sync_scores_to_redis(self) -> None:
        ...
        try:
            ...
            logger.debug(f"TokenAllocator: 已同�?{len(scores_map)} ?Token 评分�?Redis")
        except Exception as e:
            logger.warning(f"TokenAllocator: 评分同步�?Redis 失败: {e}")
