...
?10 秒向 Redis 写入当前节点心跳信息（TTL 30 秒）?
...
class NodeHeartbeat:
    ...
    ?10 秒向 Redis 写入心跳 Hash，TTL 30 �?
    心跳包含节点 ID、状态、运行时间、连接数、最?1 分钟请求�?
    """
    ...
    async def start(self) -> None:
        ...
        logger.info(f"节点心跳上报已启�?node_id={settings.node_id})")

    async def stop(self) -> None:
        """停止心跳上报并清?Redis 中的心跳数据�?""
        self._running = False
    ...
    async def get_online_nodes() -> list:
        ...
        Returns:
            在线节点列表，每个节点包?node_id、status、uptime、connections?
