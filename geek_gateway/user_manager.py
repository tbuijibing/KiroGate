...
from geek_gateway.database import user_db, User
    ...
    async def oauth_login(self, code: str) -> Tuple[Optional[User], Optional[str]]:
        ...
        if user:
            ...
            if user.approval_status != "approved":
                return None, "账号审核�? if user.approval_status == "pending" else "账号已被拒绝"
        else:
            from geek_gateway.metrics import metrics
            if await metrics.is_self_use_enabled():
                return None, "自用模式下暂不开放注�?
            # Create new user
